...
def request_url(product_id):
    ...
    for index in range(0, length):
        ...
        # return
        # dict(url=product_url,name=product_name,nickName=dict_map["nickname"],finish_time=future_time.strftime('%Y-%m-%d
        # %H:%M:%S'))
    return product_list
        ...
        for index_tmp in range(0, len(tmp)):
            if tmp[index_tmp]:
                for(k, v) in tmp[index_tmp].items():
                    tmp_data = str(k) + ":" + str(re.sub(r'\b\\U\d{3}[a-f0-9]{5}','',re.sub(r'\b\\ue\d{3}','',v)))
